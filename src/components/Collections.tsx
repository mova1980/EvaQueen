  ...
  return (
    <section id="collections" className="py-24 bg-ivory-100 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div>
            <span className={`text-caption text-gold tracking-[0.28em] block mb-3 fade-in-up ${titleVisible ? 'visible' : ''}`}>
              COLLECTIONS
            </span>
            <h2 className={`text-headline text-soft-black fade-in-up delay-1 ${titleVisible ? 'visible' : ''}`}>
              {t.collections.title}
            </h2>
            <p className={`text-body-lg text-soft-gray mt-3 max-w-md fade-in-up delay-2 ${titleVisible ? 'visible' : ''}`}>
          ...
          <div className={`hidden md:flex gap-3 flex-shrink-0 fade-in-up delay-3 ${titleVisible ? 'visible' : ''}`}>
            <button
              onClick={() => scroll('prev')}
              className="w-11 h-11 border border-stone-warm rounded-full flex items-center justify-center text-soft-gray hover:border-gold hover:text-gold hover:scale-105 transition-all duration-300"
              aria-label="قبلی"
            >
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8">
        <div className="flex items-center gap-4">
          ...
          <span className="font-en text-xs text-soft-gray/60">
            0{t.collections.items.length}
          </span>
        </div>
      </div>
    </section>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(to top, rgba(24,24,24,0.85) 0%, rgba(24,24,24,0.15) 45%, transparent 100%)',
      ...
      <div
        className="absolute inset-0 transition-opacity duration-500"
        style={{
          background: 'linear-gradient(135deg, rgba(191,163,106,0.08) 0%, transparent 70%)',
          opacity: hovered ? 1 : 0,
        }}
      />

      {/* Top frame corners — appear on hover */}
      <div
        className="absolute top-5 left-5 w-8 h-8 border-t border-l transition-all duration-500"
        style={{ borderColor: hovered ? 'rgba(191,163,106,0.6)' : 'rgba(191,163,106,0)' }}
      />
      <div
        className="absolute top-5 right-5 w-8 h-8 border-t border-r transition-all duration-500"
        style={{ borderColor: hovered ? 'rgba(191,163,106,0.6)' : 'rgba(191,163,106,0)' }}
      />

      {/* Content */}
      <div className="absolute bottom-0 left-0 right-0 p-8">
        <span className="font-en text-xs text-gold/75 tracking-[0.25em] block mb-2">
          0{index + 1}
        </span>
        <h3
          className="font-en text-xl font-light text-white mb-2 tracking-[0.06em] transition-colors duration-300"
          style={{ color: hovered ? '#F0D898' : 'rgba(247,244,239,0.97)' }}
        >
          {item.name}
        </h3>
        ...
        >
          ...
          <span className="text-caption text-gold tracking-[0.2em]">
            {/* Explore */}
            EXPLORE
          </span>
        </div>
      ...
      {/* Decorative index */}
      <div
        className="absolute top-8 end-8 font-en text-5xl font-bold pointer-events-none select-none"
        style={{ color: 'rgba(247,244,239,0.04)' }}
        aria-hidden="true"
      >
